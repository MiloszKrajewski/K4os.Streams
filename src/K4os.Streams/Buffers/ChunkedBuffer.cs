...
/// </summary>
public partial struct ChunkedBuffer<TItem>: IBuffer<TItem> where TItem: unmanaged
{
	private static int MaximumBlock0Size
	{
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		get => PooledArray<TItem>.BytesToItems(64*1024);
	}
	...
	private long Expand(long length) =>
		_chunks is not null || length > MaximumBlock0Size
			? ExpandChunks(length)
			: ExpandBlock0(length);
